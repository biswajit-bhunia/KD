...
def test_teacher_forward():
    ...
    print(f"   ✓ Teacher forward OK — shapes: emb={out['embedding'].shape}, "
...
def test_student_forward():
    ...
    print(f"   ✓ Student forward OK — shapes: emb={out['embedding'].shape}, "
          f"logits={out['logits'].shape}")
...
def test_forensic_stack():
    ...
    assert x_for.shape == (2, 6, 256, 256), f"Forensic stack shape: {x_for.shape}"
